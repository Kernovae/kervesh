...
impl App {
        ...
        ui.horizontal_wrapped(|ui| {
            ...
            if let Some(snapshot) = &tab.snapshot {
                ...
                if let Some(fs) = snapshot.filesystems.iter().find(|f| f.mount == "/") {
                    ui.monospace(format!("/ {:.0}%", fs.percent));
                    ui.separator();
                }
                let (rx, tx) = tab
                    .rates
    ...
    pub(crate) fn inspector(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new("System inspector").open(&mut open).default_width(650.0).vscroll(true).show(ctx,|ui|{
            ...
            for (name,usage) in &tab.rates.cores{ui.horizontal(|ui|{ui.monospace(name);ui.add(egui::ProgressBar::new(*usage as f32/100.0).desired_width(200.0).text(format!("{usage:.0}%")));});}
            ui.label(format!("Memory: {} used / {} total · Swap: {} used / {} total",s.memory_used().map(bytes).unwrap_or_else(||"—".into()),s.memory.get("MemTotal").copied().map(bytes).unwrap_or_else(||"—".into()),s.swap_used().map(bytes).unwrap_or_else(||"—".into()),s.memory.get("SwapTotal").copied().map(bytes).unwrap_or_else(||"—".into())));
            ...
            egui::Grid::new("mounts").striped(true).show(ui,|ui|{for label in ["Mount","Device","Used","Free","Usage"]{ui.strong(label);}ui.end_row();for fs in &s.filesystems{ui.monospace(&fs.mount);ui.label(&fs.device);ui.label(bytes(fs.used));ui.label(bytes(fs.available));ui.label(format!("{:.0}%",fs.percent));ui.end_row();}});
